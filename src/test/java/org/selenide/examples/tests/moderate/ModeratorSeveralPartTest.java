...
@DisplayName("Moderator several participants tests")
public class ModeratorSeveralPartTest {
    @Rule
    ...
    public static ParticipantsContainer pc;
    ...
    public void login() throws Exception {
        ...
        email1 = "test1" + cDate + "@test.test";
        email2 = "test2" + cDate + "@test.test";
        gp.sectionProspectsDatabase.prospectsEmailInput.setClearInput(email1, true);
        gp.sectionProspectsDatabase.prospectsEmailInput.setClearInput(email2, true);
        ...
        gp = gp.sectionReviewParticipants.performSearch(email1);
        // Invite participant
        ...
        gp = gp.sectionReviewParticipants.performSearch(email2);
        // Invite participant
        reviewdb = gp.sectionReviewParticipants.reviewProspectTable;
        reviewdb.setInvitationState("\n" +
                "          Invited\n" +
        ...
        liveStudyPage.questions.get(0).questionInput.setClearInput("Answer",false);
        liveStudyPage.questions.get(0).sendBtn.clickButton();
        liveStudyPage.questions.get(0).questionTextLocked.get(0).checkPropertyHeader("Answer");
    ...
    @DisplayName("1. Change to a different participant (and view all posts), the positive case")
    public void test1ChangeToDifferentParticipant(){
        // Open authorization page
        loginPage = open("/", LoginPageNew.class);
        // Insert login
        loginPage.setLogin(ConfProperties.getProperty("login"));
        ...
        // Check answer
        pc.participantsConversationStream.get(0).partAnswer.checkPropertyHeader("Answer");
        // Перейти на другого участника
        pc.miniCardPartMod.get(1).partNameCars.clickPropertyHeader();
        // Проверить ответ участника
        pc.participantsConversationStream.get(0).partAnswer.checkPropertyHeader("Answer");
    }
    ...
    public void logout() throws Exception {
        ...
        APIHelper.deleteProspectByMail(email1);
        APIHelper.deleteProspectByMail(email2);
        // Delete study
