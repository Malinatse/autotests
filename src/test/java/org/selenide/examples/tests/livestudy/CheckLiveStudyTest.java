...
@DisplayName("Live study tests")
public class CheckLiveStudyTest {
    @Rule
    ...
    @DisplayName("1 Check Live Study, the positive case")
    public void test1CheckLiveStudy(){
        // Click on the name
        studyEditor = controlPanel.studiesExpandablePanel.studiesLiveContainer.clickCardHeader(0);

        // Click on Moderators
        studyEditor.studySections.moderatorData.headerText.clickPropertyHeader();
        // Check name moderators
        studyEditor.studySections.moderatorData.checkModeratorName(0,ConfProperties.getProperty("adminLogin"));
        // Click on Topics&Questions
        tq = studyEditor.studySections.topicsAndQuestions;
        tq.headerText.clickPropertyHeader();
        // Check the title of the topic
        tq.topicName.checkPropertyHeader("TopicName");
    ...
    @Test
    @Category({DraftStudyGroupTests.class, PositiveTests.class, DraftStudy.class})
    @DisplayName("2 Edit Topics and questions in Live Study, the positive case")
    public void test2EditTopicsAndQuestionsInLiveStudy(){
        ...
        editTopicModal.topicNameInput.setClearInput("newTopicName",false);
        // Click save
        editTopicModal.saveTopicBtn.clickButton();
        // Edit question
        questionModal =tq.getTable(0).rows.clickCellEditButton(0);
        // Enter a question name
        questionModal.questionTitle.setClearInput("newName1",false);
        // Enter the content of the question
        questionModal.questionBody.setClearMultilinedText("newQuestion1",false);
        // Click save
        questionModal.saveQuestion.clickButton();
        // Check the title of the topic
        tq.topicName.checkPropertyHeader("newTopicName");
        // Check the id in the table
        tq.getTable(0).rows.checkTextCellData(0, VGTable.TableFields.ID, "T01 Q01");
        // Check the question in the table
        tq.getTable(0).rows.checkTextCellData(0, VGTable.TableFields.QUESTION, "newQuestion1");
    }
